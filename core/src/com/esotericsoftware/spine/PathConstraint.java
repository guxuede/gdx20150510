...
public class PathConstraint implements Updatable {
	...
	public void update () {
		...
		float[] spaces = this.spaces.ensureCapacity(spacesCount), lengths = null;
		float spacing = this.spacing;
		if (scale || lengthSpacing) {
			if (scale) lengths = this.lengths.ensureCapacity(boneCount);
	...
	float[] computeWorldPositions (PathAttachment path, int spacesCount, boolean tangents, boolean percentPosition,
		...
		float[] spaces = this.spaces.items, out = this.positions.ensureCapacity(spacesCount * 3 + 2), world;
		...
		if (!path.getConstantSpeed()) {
			...
			world = this.world.ensureCapacity(8);
		...
		if (closed) {
			verticesLength += 2;
			world = this.world.ensureCapacity(verticesLength);
		...
		// Curve lengths.
		float[] curves = this.curves.ensureCapacity(curveCount);
		float pathLength = 0;
